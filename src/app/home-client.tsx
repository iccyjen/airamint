...
// thirdweb client（就地创建，避免跨文件导出问题）
...
// —— 链上读取 —— //
const publicClient = createPublicClient({ chain: viemBase, transport: http("https://mainnet.base.org") });

async function tryReadUint(fn: string) {
  try {
    const out = await publicClient.readContract({
      address: CONTRACT as `0x${string}`,
      abi: [{ name: fn, type: "function", stateMutability: "view", inputs: [], outputs: [{ type: "uint256", name: "" }] }] as const,
      functionName: fn as any,
    });
    return BigInt(out as any);
  } catch { return null; }
}
const preferNonZero = (...vals: Array<bigint|null|undefined>) => vals.find(v => v!=null && v!==0n) ?? null;

async function fetchMintProgress() {
  const mintedBn = preferNonZero(
    await tryReadUint("totalMinted"),
    await tryReadUint("totalSupply"),
    await tryReadUint("nextTokenIdToClaim"),
  ) ?? 0n;
  const totalBn = preferNonZero(
...
function expandImgUrls(): string[] {
  ...
  if (IMG_CID) return Array.from({ length: IMG_COUNT }, (_, i) => `${IPFS_GATEWAY}/ipfs/${IMG_CID}/${i+1}.png`);
  return [];
}

export default function HomeClient() {
  // Farcaster Mini App 就绪
  ...
  useEffect(() => {
    let stop = false;
    const load = async () => { try { const p = await fetchMintProgress(); if (!stop) setProgress(p); } catch {} };
    load(); const t = setInterval(load, 20000);
    return () => { stop = true; clearInterval(t); };
  }, []);

  // 头像（只取 context.user，回退到字母圈）
  ...
  return (
    <main style={{ minHeight: "100svh", padding: 16, background: "linear-gradient(180deg,#c9dcff 0%,#b8d0ff 30%,#a9c7ff 100%)" }}>
      {/* 顶部条 */}
      ...
      <p style={{ textAlign: "center", color: "#375", opacity: 0.8, marginBottom: 16 }}>your cute onchain companions · generative collection on Base</p>

      {/* 主图卡片 */}
      <div style={{ maxWidth: 420, margin: "0 auto 16px", background: "#bfe6ff", borderRadius: 16, height: 360, display: "grid", placeItems: "center", boxShadow: "0 10px 20px rgba(85,120,255,.25)", border: "1px solid rgba(80,110,255,.3)" }}>
        {imgs.length ? (
          <img
            key={imgs[idx]} src={imgs[idx]} alt="collection"
            style={{ width: "82%", height: "82%", objectFit: "cover", borderRadius: 12 }}
            onError={() => setBadSet(prev => { const ns = new Set(prev); ns.add(imgs[idx]); return ns; })}
          />
        ) : (
          <div style={{ opacity: 0.6, fontWeight: 600, color: "#557", padding: 12, textAlign: "center" }}>
            图片加载失败或未配置。请设置 <b>NEXT_PUBLIC_IMG_LIST</b>（完整 URL，逗号分隔）
            或 <b>NEXT_PUBLIC_IMG_CID</b> + <b>NEXT_PUBLIC_IMG_COUNT</b>。
          </div>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, maxWidth: 420, margin: "0 auto" }}>
        <button
          onClick={() => sdk.actions.composeCast({ text: "我刚在 Base 铸了一枚 NFT（0.001 ETH）#MintU", ...(appUrl ? { embeds: [appUrl] } : {}) })}
        ...
        {isConnected ? (
          <ClaimButton
            ...
            onTransactionConfirmed={(tx) => {
              ...
              if (appUrl) sdk.actions.composeCast({ text: "我刚在 Base 铸了一枚 NFT（0.001 ETH）#MintU", embeds: [appUrl] });
            }}
      ...
      <div style={{ maxWidth: 420, margin: "12px auto 0", textAlign: "center", color: "#334" }}>
        {isConnected ? (
          <p>Base 余额： <b>{balance ? Number(balance.formatted).toFixed(4) : "--"} {balance?.symbol ?? "ETH"}</b></p>
        ) : (
          <p style={{ opacity: 0.8 }}>请先连接钱包（在 Warpcast Mini App 中打开）</p>
        )}
        {/* 交易成功提示（txHash 在上面设置过） */}
      </div>
    </main>
  );
}
